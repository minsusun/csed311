...
void print_pvs(const std::unique_ptr<Vtop>& cp) {
    ...
    string reg_hex, mem_hex;
    stringstream reg_value, mem_value;

    cout << "Cycle: " << dec << total_cycle << endl;
    cout << left << setw(8) << "is_halted: " << bitset<1>(cp->is_halted) << endl;
    cout << left << setw(8) << "pc: " << hex << cp->print_pc << endl;
    cout << endl << left << setw(8) << "Register outputs" << endl;
    for(int i = 0; i < 32; i++) {
    ...
    for(int i = 0; i < 11; i++) {
        mem_value << setw(8) << setfill('0') << hex << cp->print_mem[i];
        mem_hex = mem_value.str();
        mem_value.str("");

        cout << setw(2) << hex << 0x2fdc + 4 * i << " " << mem_hex << endl;
    }
